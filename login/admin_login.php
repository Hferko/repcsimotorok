    ...
    <main>
        ...
        if (!isset($_SESSION["admin"])) {
            ...
            if (isset($_POST['nev']) && isset($_POST['password'])) {
                ...
                $sql = "SELECT * FROM `felhasznalo` WHERE neve='$nev' AND `status`='admin';";
                $eredmeny = mysqli_query($conn, $sql) or die(mysqli_error($conn));

                if (!$eredmeny) {
                    echo "Ez a hiba: " . mysqli_error($conn) . ' ' . mysqli_errno($conn);
                    echo "<div class='form'><h3>A felhasználónév/jelszó helytelen.</h3><br/>Kattintson ide a  <a href='admin_login.php'>Belépéshez</a></div>";
                } else {
                    if (mysqli_num_rows($eredmeny) === 1) {
                        $sor = mysqli_fetch_assoc($eredmeny);

                        if (password_verify($jelszo, $sor['password'])) {
                            $_SESSIO['admin'] = $sor['nev'];
                        } else {
                        ...
                        print('<h4>Önnek nincs Admin jogosultsága<br>kérem folytassa a vásárlást<br><br></h4>');
                        echo '<button><a href="../shop.php"><b>VISSZA A WEBÁRUHÁZBA</b></a></button>';
                    }
                }
            } else {
        ?>
                <div class="card">
                    <div class="form">
                        ...
                        <p>A továbblépéshez admin jogosultságok szükségesek</p>
                        ...
                        <!--
			<p>Ön még nem regisztrált felhasználó? <br></p> <h4><a href='regisztral.php'>&nbsp; KÉRJÜK ITT REGISZTRÁLJON </a></h4>
-->
                    </div>
            <?php
            }
